...
// Case 1 : Calculer la moyenne de la surface et valeur foncière par département

object transformation_1 extends App{
  ...
  val streamSpark = streamingDataFrameImmo.withColumn("timestamp", unix_timestamp(col("date_mutation"), "y-M-d"))
    ...
    .agg(avg(col("surface_reelle_bati")).alias("surface_reelle_bati_avg"), avg(col("valeur_fonciere")).alias("valeur_fonciere_avg"))
    .select(col("code_departement"), round(col("surface_reelle_bati_avg"), 2).alias("surface_reelle_bati_avg"), round(col("valeur_fonciere_avg")).alias("valeur_fonciere_avg"))
    .orderBy(col("code_departement"))
